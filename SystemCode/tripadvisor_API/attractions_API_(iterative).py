...
# This script iteratively calls the tripadvisor API for ATTRACTIONS.
# Use restaurants_API script instead to call the API for local restaurants.
...
# 3) Run the script again, this time updating 'start' and 'end' to a higher value. The script automatically
#    appends data to the same file. Be careful not to create duplicates.
...
# API call parameters
URL = r'https://tripadvisor1.p.rapidapi.com/attractions/list'
...
param = {
    ...
    # How results will be sorted. Stick to ranking.
    "sort": "ranking",
...
jsonDir = r'Attraction_JSONs\XYZ\Attractions.json'
...
for offset in range(start, end):
    ...
    if offset == start:
        ...
        jsonDir = 'Attraction_JSONs\\'+ loc + '\\Attractions.json'
...
try:
    if os.stat(jsonDir).st_size > 0:
        print ("Attractions.json already exists.")
        try:
...
with open(logDir, 'a') as file:
    if newFileFlag == True:
        file.write("Attractions.json created with offset %d to %d (entries %d to %d) at %s.\n" % (start,end,start*30,end*30-1,ctime()))
    else:
        file.write("Attractions.json updated with offset %d to %d (entries %d to %d) at %s.\n" % (start,end,start*30,end*30-1,ctime()))
